<template>
  <div>
    <el-input :disabled="isReadOnly" @blur.native.capture="onBlur()" size="small" v-show="isShow" v-model="value" :name="field.vModel" :clearable="clearable" :style="field.style" :placeholder="field.fieldRemark" class="input-with-select">
      <el-button id="selectId" v-if="isReadOnly" slot="append" @click="onlineForm_onopenHyperlink" type="primary" title="点击查看" icon="el-icon-link"></el-button>
    </el-input>
  </div>
</template>
    ...
    methods: {
      ...
      onlineForm_onopenHyperlink(thisObj, field) {
        ...
        if (that.inputCtrlOption) {
          ...
          if (linktype == 'customJS') {
            var custFuncName = hyperlinkUrl;
            if (hyperlinkUrl && Object.toIsExitsFunction(custFuncName)) {
              let parms = {};
              let isSubField = that.rowindex != undefined && that.rowindex != null;
              if (isSubField) {
                let row = that.$parent.data;
              ...
              window[custFuncName](that.value, fieldDef, parms);
            }
            return false;
          }
          else if (linktype == 'wfsheet') {
            // 换回wiid
