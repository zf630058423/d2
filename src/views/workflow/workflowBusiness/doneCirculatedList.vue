      ...
      <el-main class="f2bpm-page-right">
        <div>
          ...
          <div id="grid-toolbar" class="grid-toolbar">
              ...
              <el-button v-if="searchParams.isHistory==0" size="small" type="default" @click="setDoingCirculated">
                  {{$t('pageInfo.markunread')}}
              </el-button>
              <!-- 导出 -->
          ...
          <div>
            <f2bpm-datagrid :expand-search="expandSearch" ref="grid" :show-checkbox="true" :show-seq="true" height="full-190" grid="grid" :pager-config="tablePage" :data="tableData" :columns="tableColumn" @page-change="pageChange" @sort-change="sortChange" />
          </div>
        </div>
      </el-main>
    </el-container>
  ...
  export default {
    ...
    methods: {
      ...
      getGridData() {
        ...
        var res = Object.toAjaxJson(FUI.Handlers.WorkflowBusinessHandler, 'getDoneCirculatedListJson', parm, true, null, function (res) {
          if (res.success) {
      ...
      setDoingCirculated() {
        ...
        if (taskIds == "" || taskIds == "") {
          FUI.Window.showMsg(that, "请选择要标记的行");
          return;
        }
        FUI.Window.confirm(that, '确定要还原标记为未阅吗？', '温馨提示', function (index) {
          try {
            var dataJsonParm = {};
            dataJsonParm.taskId = taskIds;
            dataJsonParm.isHiTask = isHistory;
            var mssg = Object.toAjaxJson(workflowformSender, 'setUnDoneCirculated', dataJsonParm);
      ...
      excelExportClick() { 
        ...
        Object.toAjaxDownloadFile('/workflow/workflowExport/', 'exportDoneCirculated', parm);
      },
      openWorkflowfromView(appId, formAction, wiid, workflowTitle) {
        var that = this;
        openWorkflowfromView(this, appId, wiid, workflowTitle,'','_blank');  //_self
      }
    }
